...
SubDataObject::SubDataObject(std::vector<float> &verticies, std::vector<unsigned> &indicies, Transform &aTransform) : transform(aTransform)
{
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 verticies.size() * sizeof(float) + indicies.size() * sizeof(unsigned), 0, GL_STATIC_DRAW);

    GLsizeiptr currentoffset = 0;
    glBufferSubData(GL_ARRAY_BUFFER, currentoffset, verticies.size() * sizeof(float), &verticies[0]);
...
void SubDataObject::Draw(Shader *shader, std::vector<float> &verticies, std::vector<unsigned> &indicies)
{
